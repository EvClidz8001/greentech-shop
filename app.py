    ...
    products = [
        ...
        products = [
            Product(name="Индикатор почвы «Бумажка»", price=390, 
                   ...
                   image_emoji="📄", category="indicator"),
            
            Product(name="Умный полив «AquaBot Pro»", price=2490, 
                   description="Вставляется в грунт. Полив активируется через мобильное приложение. Таймер от 1 минуты до 24 часов. Присылает push-уведомление, если влажность почвы ниже 20%. Совместим с iOS/Android. Разработка на основе технологии NASA (1980).", 
                   image_emoji="💧", category="watering"),
            
            Product(name="Спрей «Био-Щит»", price=590, 
                   description="Натуральный состав против паутинного клеща и тли. Экстракт пиретрума и масло нима (проверено в лаборатории защиты растений РАН). Также доступен спрей 'Рост' с морскими водорослями для активного роста.", 
                   image_emoji="🌿", category="spray"),
            
            Product(name="Корневой анализатор «RootScan»", price=3990, 
                   description="Устанавливается на дно горшка. Анализирует состояние корневой системы. Если корням холодно — датчик показывает синий (нужно повысить температуру), если мало света — оранжевый (нужно больше освещения). Методы в инструкции.", 
                   image_emoji="🔬", category="analyzer"),
            
            Product(name="UV-лампа «Фотосинтез-660»", price=4990, 
                   description="Ультрафиолетовая лампа полного спектра с пиком 660 нм для ускорения фотосинтеза. Встроенный таймер 12/16 часов. Рекомендации от ботаников МГУ (лаборатория фотосинтеза, 2022).", 
                   image_emoji="💡", category="light"),
            
            Product(name="Датчик влажности «Drop Sensor»", price=1290, 
                   description="Анализирует состав влаги и минералов. Передает данные на смартфон через Bluetooth. Предупреждает о засухе за 24 часа. Используется в теплицах Агрофизического института.", 
                   image_emoji="📱", category="sensor")
        ]
